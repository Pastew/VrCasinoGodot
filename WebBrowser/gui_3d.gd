...
func _ready():
	...
	#right_controller.button_pressed.connect(self._on_left_controller_button_pressed)
	#right_controller.button_released.connect(self._on_left_controller_button_released)
...
func _on_left_controller_button_pressed(button: String) -> void:
	send_mouse_move_to_subviewport()
	send_mouse_click_to_subviewport(true)
 
func _on_left_controller_button_released(button: String) -> void:
	send_mouse_move_to_subviewport()
	send_mouse_click_to_subviewport(false)
...
func _input(ev):
	if Input.is_key_pressed(KEY_S):
		emulateClickOnSpinButton()

func emulateClickOnSpinButton():	
	send_mouse_move_to_subviewport()
	send_mouse_click_to_subviewport(true)
	send_mouse_click_to_subviewport(false)

func send_mouse_click_to_subviewport(pressed: bool):
	var mouse_event = InputEventMouseButton.new()
	mouse_event.button_index = MOUSE_BUTTON_LEFT
	mouse_event.pressed = pressed  # True for mouse button press, False for release
	# mouse_event.button_mask = MOUSE_BUTTON_MASK_LEFT
	
	var position = get_spin_button_position()
	mouse_event.position = position
	mouse_event.global_position = position

	node_viewport.push_input(mouse_event)
	
func send_mouse_move_to_subviewport():
	var position = get_spin_button_position();
